		...
		if (! layerm->slices.empty()) {
			...
			if (config.ironing_type != IroningType::NoIroning &&
			    (config.ironing_type == IroningType::AllSolid ||
				    ((config.top_shell_layers > 0 || (this->object()->spiral_mode_enabled() && config.bottom_shell_layers > 1)) &&
		...
		if (ironing_params.just_infill) {
			...
			for (size_t k = i; k < j; ++ k) {
				...
				if (iron_completely) {
					...
					for (const Surface &surface : ironing_params.layerm->slices.surfaces)
						if ((surface.surface_type == stTop && (region_config.top_shell_layers > 0 || this->object()->spiral_mode_enabled())) || (iron_everything && surface.surface_type == stBottom && region_config.bottom_shell_layers > 0))
